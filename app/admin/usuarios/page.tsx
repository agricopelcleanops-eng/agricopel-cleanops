      ...
      <div className="max-w-5xl mx-auto">
        ...
        <form onSubmit={cadastrarUsuario} className="bg-slate-900 p-8 rounded-2xl mb-10 border border-slate-800 shadow-2xl">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="flex flex-col gap-2">
                <label className="text-[10px] font-bold text-slate-500 uppercase">Usuário / Nick</label>
                <input type="text" placeholder="ex: deise" value={usuario} onChange={(e) => setUsuario(e.target.value)} className="bg-slate-800 p-3 rounded-lg border border-slate-700 focus:border-orange-500 outline-none text-sm" required />
            </div>
            <div className="flex flex-col gap-2">
                <label className="text-[10px] font-bold text-slate-500 uppercase">Nível</label>
                <select value={nivel} onChange={(e) => setNivel(e.target.value)} className="bg-slate-800 p-3 rounded-lg border border-slate-700 outline-none text-sm">
            ...
            <div className="flex flex-col gap-2">
                ...
                <input type="text" value={unidade} onChange={(e) => setUnidade(e.target.value)} className="bg-slate-800 p-3 rounded-lg border border-slate-700 outline-none text-sm" />
            </div>
            <div className="flex flex-col gap-2 justify-end">
                <button className="bg-orange-600 hover:bg-orange-500 font-black py-3 rounded-lg transition-all text-sm uppercase">Cadastrar</button>
            </div>
          </div>
        ...
        <div className="bg-slate-900 rounded-2xl overflow-hidden border border-slate-800">
          <table className="w-full text-left">
            <thead className="bg-slate-800/50 text-slate-500 uppercase text-[10px] font-black">
              <tr><th className="p-5">Usuário</th><th className="p-5">Cargo</th><th className="p-5">Unidade</th><th className="p-5 text-right">Ação</th></tr>
            </thead>
            <tbody className="divide-y divide-slate-800">
              {usuarios.map((u) => (
                <tr key={u.id} className="hover:bg-slate-800/30">
                  <td className="p-5 text-sm text-slate-200">{u.email}</td>
                  <td className="p-5"><span className="px-3 py-1 rounded-full text-[9px] font-black uppercase bg-slate-800">{u.nivel}</span></td>
                  <td className="p-5 text-xs text-slate-500 font-bold">{u.unidade}</td>
                  <td className="p-5 text-right"><button onClick={() => excluirUsuario(u.id, u.email)} className="text-[10px] font-black text-slate-500 hover:text-red-500">Remover</button></td>
                </tr>
              ))}
            </tbody>
          </table>
