      ...
      <main className="max-w-4xl mx-auto p-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <button onClick={() => setMostrarForm(!mostrarForm)}
            className="bg-orange-500 hover:bg-orange-600 text-white font-semibold px-4 py-2 rounded-xl text-sm">
            + Novo usuário
          </button>
        </div>
            ...
            <form onSubmit={criarUsuario} className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input value={novoNome} onChange={e => setNovoNome(e.target.value)} required
                  ...
                  placeholder="Ex: Deise Silva" />
              </div>
              <div>
                <label className="block text-xs font-bold text-gray-400 uppercase mb-1">Login (nickname)</label>
                <input value={novoLogin} onChange={e => setNovoLogin(e.target.value)} required
                  className="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-orange-500"
                  placeholder="Ex: deise" autoCapitalize="none" />
                {novoLogin && <p className="text-xs text-gray-400 mt-1">Login: <strong>{novoLogin.toLowerCase().replace(/\s+/g,'')}</strong></p>}
              </div>
              <div>
              ...
              <div className="col-span-2 flex gap-3 justify-end">
                <button type="button" onClick={() => setMostrarForm(false)}
                  className="px-4 py-2 border border-gray-200 rounded-xl text-sm text-gray-600">Cancelar</button>
          ...
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="text-left px-4 py-3 text-xs font-bold text-gray-400 uppercase">Login</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {carregando ? (
                ...
                <tr key={u.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-gray-500 text-sm font-mono">{u.login || u.email?.split('@')[0]}</td>
                  ...
                  <td className="px-4 py-3">
                    <button onClick={() => alterarAtivo(u.id, !u.ativo)}
                      className={`text-xs font-bold px-3 py-1 rounded-full ${u.ativo ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
